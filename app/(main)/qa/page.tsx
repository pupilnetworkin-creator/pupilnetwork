    ...
    <div className="max-w-5xl mx-auto space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">Stuck? Ask a question. Know the answer? Earn points!</p>
        </div>
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm space-y-4">
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
          ...
          <div className="flex items-center gap-2 w-full md:w-auto overflow-x-auto pb-1 hide-scrollbar">
            <span className="text-sm font-medium text-slate-500 mr-2 shrink-0">Subject:</span>
            {['All', ...uniqueSubjects].map((sub) => (
              <Button
                ...
                className={`rounded-full shrink-0 h-8 px-3 text-xs ${subjectFilter === sub ? 'bg-indigo-600' : 'bg-slate-100 text-slate-600'}`}
              >
                {sub}
              </Button>
            ))}
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          <div className="text-center py-24 bg-white rounded-2xl border border-slate-100 border-dashed">
            ...
            <p className="text-slate-400 text-sm mt-1">Be the first to ask in this category!</p>
          </div>
        )}
      </div>
    </div>
