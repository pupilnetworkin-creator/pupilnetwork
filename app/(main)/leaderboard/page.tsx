  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="text-center py-8">
        ...
        <h1 className="text-4xl font-bold text-slate-900 mb-4" style={{ fontFamily: 'Space Grotesk, sans-serif' }}>
          Global Leaderboard
        </h1>
        ...
        <div className="hidden md:flex justify-center items-end gap-6 mb-12 mt-8 px-4">
          ...
          <div className="flex flex-col items-center">
            <div className="relative mb-4">
              <Avatar className="w-20 h-20 border-4 border-slate-200 shadow-md">
              ...
              <div className="absolute -bottom-3 left-1/2 -translate-x-1/2 w-8 h-8 bg-slate-200 rounded-full flex items-center justify-center text-slate-600 font-bold shadow-sm border-2 border-white">2</div>
            </div>
            <Link href={`/profile/${topUsers[1].username}`} className="font-bold text-slate-800 hover:text-indigo-600 truncate max-w-[120px] text-center">
              {topUsers[1].display_name}
              {topUsers[1].is_premium && <Crown className="w-3.5 h-3.5 text-yellow-500 inline ml-1" />}
            </Link>
            <span className="text-indigo-600 font-bold mt-1">{formatPoints(topUsers[1].points)} pts</span>
            <div className="w-24 h-24 bg-gradient-to-t from-slate-100 to-slate-50 mt-4 rounded-t-xl border border-b-0 border-slate-200" />
          </div>
          ...
          <div className="flex flex-col items-center z-10">
            <div className="relative mb-4">
              ...
              <div className="absolute -bottom-4 left-1/2 -translate-x-1/2 w-10 h-10 bg-yellow-400 rounded-full flex items-center justify-center text-yellow-900 font-black shadow-md border-2 border-white text-lg">1</div>
            </div>
            <Link href={`/profile/${topUsers[0].username}`} className="font-bold text-slate-900 text-lg hover:text-indigo-600 truncate max-w-[150px] text-center">
              {topUsers[0].display_name}
              {topUsers[0].is_premium && <Crown className="w-4 h-4 text-yellow-500 inline ml-1.5" />}
            </Link>
            <span className="text-indigo-600 font-bold mt-1 text-lg">{formatPoints(topUsers[0].points)} pts</span>
            <div className="w-32 h-32 bg-gradient-to-t from-amber-100 to-yellow-50 mt-4 rounded-t-2xl border border-b-0 border-yellow-200 shadow-[0_-10px_20px_-10px_rgba(251,191,36,0.3)]" />
          </div>
      ...
      <Card className="border-slate-200 shadow-sm overflow-hidden mb-12">
        <div className="bg-slate-50 border-b border-slate-100 px-6 py-4 flex items-center justify-between gap-4 flex-wrap">
          <h2 className="font-semibold text-slate-800">Top 50 Students</h2>
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-4 px-4 sm:px-6 py-4 transition-colors ${
                    isMe ? 'bg-indigo-50/50 hover:bg-indigo-50/80' : 'hover:bg-slate-50'
                  }`}
                >
                  <div className={`w-8 text-center font-bold ${
                    rank === 1 ? 'text-yellow-500 text-lg' :
                    rank === 2 ? 'text-slate-400 text-lg' :
                    rank === 3 ? 'text-amber-700 text-lg' : 'text-slate-400'
                  }`}>
                    #{rank}
                  </div>

                  <Link href={`/profile/${profile.username}`} className="flex-1 flex items-center gap-3 min-w-0">
                    <Avatar className="w-10 h-10 shrink-0 border border-white shadow-sm ring-1 ring-slate-100">
                    ...
                    <div className="min-w-0">
                      <p className="font-semibold text-slate-900 truncate flex items-center gap-1.5">
                        ...
                        {isMe && <span className="text-[10px] uppercase tracking-wider font-bold text-indigo-400 bg-indigo-50 px-1.5 py-0.5 rounded-full">You</span>}
                      </p>
                      <p className="text-xs text-slate-500 truncate">@{profile.username}</p>
                    </div>
                  </Link>

                  <div className="flex items-center gap-1.5 bg-indigo-50 text-indigo-700 px-3 py-1.5 rounded-full font-bold text-sm border border-indigo-100 shrink-0">
